...
public class ModKeyMappings {
    ...
    public static KeyMapping ACTIVE_CURIO_SKILL = new KeyMapping("key.revelationfix.curios_skill", GLFW.GLFW_KEY_K, "key.revelationfix.category");
    public static KeyMapping ACTIVE_ARMOR_SKILL = new KeyMapping("key.revelationfix.armor_skill", GLFW.GLFW_KEY_N, "key.revelationfix.category");
    ...
    public static void onKeyboard(InputEvent.Key event) {
        ...
        if (mc.screen == null) {
            if (event.getAction() == GLFW.GLFW_PRESS) {
                if (key == ACTIVE_CURIO_SKILL.getKey().getValue()) {
                ...
                if (key == ACTIVE_ARMOR_SKILL.getKey().getValue()) {
                    Player player = ClientWrapped.clientPlayer();
                    for (ItemStack itemStack : player.getArmorSlots()) {
                        if (itemStack.getItem() instanceof IModeToggleItem) {
                            IModeToggleItem.send(itemStack.getItem());
                            break;
                        }
                    }
                }
